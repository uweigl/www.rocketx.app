...
(flat fee vs GMV %, integration waived, billing at go-live). The one exception
...
C["en"] = dict(
    ...
    native=[("Reordering is a habit, not a search", "Most B2B orders are repeat orders. A browser makes the buyer find a bookmark, load a site, and log in every time. An app is one tap, already signed in — friction removed from every reorder for the life of the account."),
            ...
            ("And the browser stays", "A full web shop runs in any browser on any computer — the same catalogue, the same server-side cart. The apps take over where the browser cannot go.")],
    ...
    terms=[("Flat fee, never a percentage of GMV", "Platform cost does not rise because your order volume did. Growth is not taxed."),
           ...
           ("Integration fee waived on annual plans", "The implementation cost that usually front-loads these projects is removed."),
           ("Billing starts at go-live, not at signature", "You pay nothing until the platform is live and delivering value."),
           ("30–45 day pilot with joint ROI review", "A defined evaluation window with agreed measures, before a longer commitment. Miss the measures, and you walk away owing nothing."),
    ...
    xs=[('The problem', 'Your buyers assemble orders somewhere nobody on your side can see, in a browser tab they have to find and log into every time - while the work itself happens on a phone, in an aisle, at a loading dock. Carts die with the session, one person builds what six to ten people decide, and the duplicate nobody caught comes back as a return.'),
        ...
        ('What it costs to try', "Integration waived on annual plans. Billing starts at go-live, not at signature. A 30 to 45 day pilot with agreed measures — miss them, and you walk away owing nothing."),
    ...
    eu=[("Counterparty", "An Irish company, incorporated and operating in the European Union under EU law, separate from the US entity. RocketX Limited, Company No. 344153, registered office Derrylahan, Ballyhaunis, Co. Mayo, F35 W667, Ireland."),
    ...
    us=[("Counterparty", "RocketX LLC, the United States entity - an Arizona limited liability company, Business ID 25040687. Same platform, same commitments, governed where you are."),
    ...
    at=['Data processing agreement, with roles written down', 'Current subprocessor list, with change notice', "Hosting regions for your data, named in writing", 'Uptime and support commitments, by tier', 'Retention, deletion and backup policy'],
    ...
    foot="RocketX · Frictionless Ordering · www.rocketx.app",
    pg="Page",
)
...
C["de"] = dict(
    ...
    native=[("Nachbestellen ist Gewohnheit, keine Suche", "Die meisten B2B-Bestellungen sind Wiederholungsbestellungen. Im Browser muss der Einkäufer jedes Mal das Lesezeichen finden, die Seite laden und sich anmelden. Eine App ist ein Fingertipp entfernt, bereits angemeldet — diese Reibung entfällt dauerhaft bei jeder Nachbestellung."),
            ...
            ("Und der Browser bleibt", "Ein vollwertiger Webshop läuft in jedem Browser auf jedem Rechner — derselbe Katalog, derselbe serverseitige Warenkorb. Die Apps übernehmen dort, wo der Browser nicht hinkommt.")],
    ...
    terms=[("Pauschalgebühr, niemals ein Umsatzprozentsatz", "Die Plattformkosten steigen nicht, weil Ihr Bestellvolumen gestiegen ist. Wachstum wird nicht besteuert."),
           ...
           ("Integrationsgebühr bei Jahresplänen erlassen", "Die Implementierungskosten, die solche Projekte sonst vorbelasten, entfallen."),
           ("Abrechnung ab Go-live, nicht ab Unterschrift", "Sie zahlen nichts, bis die Plattform live ist und Wert liefert."),
           ("30–45-Tage-Pilot mit gemeinsamem ROI-Review", "Ein definierter Bewertungszeitraum mit vereinbarten Kennzahlen, vor einer längeren Bindung. Verfehlt der Pilot die Kennzahlen, gehen Sie und zahlen nichts."),
    ...
    xs=[('Das Problem', "Ihre Einkäufer stellen Bestellungen dort zusammen, wo sie auf Ihrer Seite niemand sieht. In einem Browser-Tab, den sie jedes Mal suchen. In den sie sich jedes Mal neu anmelden. Und die Arbeit selbst passiert woanders: am Telefon, im Lagergang, an der Laderampe. Warenkörbe sterben mit der Sitzung. Einer baut, was sechs bis zehn entscheiden. Und die übersehene Dopplung kommt als Retoure zurück."),
        ...
        ('Was ein Versuch kostet', "Integration bei Jahresplänen erlassen. Abrechnung ab Go-live, nicht ab Unterschrift. Ein 30- bis 45-tägiger Pilot mit vereinbarten Kennzahlen: Verfehlt er sie, zahlen Sie nichts."),
    ...
    eu=[("Vertragspartner", "Eine irische Gesellschaft, gegründet und tätig in der Europäischen Union nach EU-Recht, getrennt von der US-Gesellschaft. RocketX Limited, Company No. 344153, Sitz: Derrylahan, Ballyhaunis, Co. Mayo, F35 W667, Irland."),
    ...
    us=[("Vertragspartner", "Die RocketX LLC, die US-Gesellschaft - eine Limited Liability Company nach dem Recht Arizonas, Business ID 25040687. Dieselbe Plattform, dieselben Zusagen, geltendes Recht an Ihrem Sitz."),
    ...
    at=['Auftragsverarbeitungsvertrag mit schriftlich fixierten Rollen', 'Aktuelle Liste der Unterauftragsverarbeiter, mit Änderungsmitteilung', "Hosting-Regionen Ihrer Daten, schriftlich benannt", 'Zusagen zu Verfügbarkeit und Support, nach Stufe', 'Aufbewahrungs-, Lösch- und Backup-Richtlinie'],
    ...
    nf=[('Sie verkaufen ausschließlich an Endkunden', "Der größte Teil des Produkts ist Großhandelsmechanik, die ein Endkundenkatalog nie berührt."),
    ...
    foot="RocketX · Reibungsloses Bestellen · www.rocketx.app",
    pg="Seite",
)

C["es"] = dict(
    lang="es", file="rocketx-business-case-es",
    doctitle="RocketX — Caso de negocio para pedidos B2B modernos",
    kicker="CASO DE NEGOCIO INTERNO",
    title="Caso de negocio para pedidos B2B modernos",
    sub="Para equipos de venta mayorista, distribución y manufactura que evalúan cómo sus clientes hacen pedidos.",
    ...
    stats=[("US$ 860.000 M", "de comercio electrónico B2B en América Latina en 2025, creciendo cerca de un 24 % al año"),
           ("24 %", "de crecimiento anual previsto hasta 2033: la región de comercio electrónico que más rápido crece del mundo"),
    ...
    p1b="La consecuencia práctica: la experiencia de pedido ya no es un sistema de trastienda. Es la superficie por la que tus clientes te juzgan, y el punto en el que deciden si repiten o llaman a otro.",
    ...
    costs=[("Rendimiento del catálogo", "Las plataformas creadas para el comercio minorista se ralentizan con catálogos mayoristas. Los compradores abandonan la búsqueda y llaman a un vendedor, convirtiendo un pedido de autoservicio en uno atendido."),
           ...
           ("Comisiones sobre las ventas", "Las tarifas ligadas al volumen hacen que el costo de la plataforma suba justo cuando tienes éxito. El crecimiento se grava en vez de apoyarse."),
    ...
    caps=[("SKUs ilimitados, búsqueda en menos de un segundo", "Tanto si el catálogo tiene ochocientos SKUs como ocho millones, filtre y pida en milisegundos — y nunca pague por SKU. Un rendimiento que otras plataformas no igualan a esta escala."),
          ...
          ("Primero la venta mayorista, y con ella tu canal de consumo", "El producto está construido en torno al pedido mayorista. Si además vendes directo al consumidor, ese canal se alimenta del mismo catálogo y de la misma conexión con el ERP. Ni una segunda plataforma, ni un segundo juego de datos de producto."),
    ...
    native=[("Recomprar es un hábito, no una búsqueda", "La mayoría de los pedidos B2B son repeticiones. En un navegador el comprador debe encontrar el marcador, cargar el sitio e iniciar sesión cada vez. Una app está a un toque, con la sesión iniciada — fricción eliminada en cada recompra, de forma permanente."),
            ("Una posición permanente en el teléfono del comprador", "Tu icono queda en la pantalla de inicio. A la competencia hay que buscarla y redescubrirla."),
            ...
            ("Sobrevive a la pérdida de señal", "Los almacenes con paredes de acero y las rutas rurales acaban con la cobertura. Catálogo, precios, carrito y tus folletos y vídeos de producto quedan en local y sincronizan al volver la conexión."),
            ("Y el navegador se queda", "Una tienda web completa funciona en cualquier navegador y en cualquier computadora — el mismo catálogo, el mismo carrito en el servidor. Las apps toman el relevo donde el navegador no llega.")],
    p5h="Cómo las condiciones comerciales reducen el riesgo",
    p5a="La estructura del acuerdo está diseñada para que la plataforma se demuestre antes de costar nada. Aquí no se citan importes — los precios vigentes están en el sitio web y en tu propuesta. El gráfico de abajo solo muestra cuánto representa la cuota como parte de tus ventas.",
    terms=[("Tarifa fija, nunca un porcentaje de las ventas", "El costo de la plataforma no sube porque suba tu volumen de pedidos. El crecimiento no se grava."),
           ...
           ("Tarifa de integración incluida en planes anuales", "Se elimina el costo de implementación que normalmente carga estos proyectos por adelantado."),
           ...
           ("Piloto de 30–45 días con revisión conjunta de ROI", "Una ventana de evaluación definida con métricas acordadas, antes de un compromiso más largo. Si el piloto no las alcanza, te vas sin deber nada."),
    ...
    steps=[("1. Auditoría de los sistemas", "Mapeamos el catálogo actual, las reglas de precios, el ERP y el flujo de pedidos, e identificamos qué debe migrar y qué se queda."),
           ("2. Integración", "Se construye la sincronización bidireccional contra tu ERP y CRM. Los conectores estándar cubren los sistemas principales; lo que exceda ese alcance se cotiza como un bloque de trabajo definido."),
    ...
    cart=[("Un pedido a medio construir es invisible", "Alrededor del 70 % de los carritos online se abandonan, y en la mayoría de plataformas nadie de tu lado puede ver uno hasta que se convierte en pedido. Un carrito atascado se ve igual que ningún carrito: te enteras de que existía cuando nunca llega."),
          ...
          ("Qué hace RocketX en su lugar", "Los carritos persisten en el servidor y se sincronizan en vivo entre la web app de RocketX en cualquier navegador y las apps nativas. Tus vendedores ven los carritos de cliente en vivo, incluido lo que se ha atascado, y pueden actuar antes de que se enfríe. Todo el equipo de compras edita un carrito con cada cambio atribuido. Y el carrito avisa de un SKU que la cuenta ya compró hace poco, antes de enviar el pedido.")],
    ...
    xs=[('El problema', "Tus compradores arman pedidos donde nadie de tu lado los ve: en una pestaña del navegador que deben encontrar y en la que deben identificarse cada vez. Y el trabajo ocurre en otro sitio — en un móvil, en un pasillo de almacén, en un muelle de carga. Los carritos mueren con la sesión. Una persona construye lo que deciden entre seis y diez. Y el duplicado que nadie vio vuelve como devolución."),
        ('Qué cambia en el carrito', 'Los carritos persisten en el servidor y se sincronizan en vivo entre la web app de RocketX en cualquier navegador y las apps nativas. Tus vendedores ven los carritos de cliente en vivo, incluido lo que se ha atascado, y pueden actuar antes de que se enfríe. Todo el equipo de compras edita un solo pedido, con cada cambio atribuido. Y el carrito avisa de un SKU que la cuenta ya compró hace poco, antes de enviar.'),
        ...
        ('Qué cuesta probarlo', "Integración incluida en planes anuales. La facturación empieza en la puesta en marcha, no en la firma. Piloto de 30 a 45 días con métricas acordadas: si no las alcanza, no debes nada."),
    ...
    cmh="Dos entidades, dos jurisdicciones, una respuesta para cada una",
    ...
    eu=[("Contraparte", "Una sociedad irlandesa, constituida y operativa en la Unión Europea bajo derecho de la UE, separada de la entidad estadounidense. RocketX Limited, Company No. 344153, domicilio social: Derrylahan, Ballyhaunis, Co. Mayo, F35 W667, Irlanda."),
    ...
    us=[("Contraparte", "RocketX LLC, la entidad estadounidense - una sociedad de responsabilidad limitada de Arizona, Business ID 25040687. La misma plataforma, los mismos compromisos, con la ley de donde estás."),
    ...
    at=['Acuerdo de tratamiento de datos, con los roles por escrito', 'Lista vigente de subencargados, con aviso de cambios', "Regiones de alojamiento de tus datos, por escrito", 'Compromisos de disponibilidad y soporte, por nivel', 'Política de retención, borrado y copias de seguridad'],
    ...
    tlh="Cuánto se tarda en usarlo de verdad",
    ...
    tln="Las cifras de competidores son referencias de mercado medio hasta la puesta en marcha, según guías de plataformas y agencias de 2026. El alcance las mueve en ambos sentidos y el ERP es donde más se desbordan. La de RocketX corresponde a un piloto en vivo con un grupo definido de compradores, no al despliegue completo. En las alternativas la facturación empieza mucho antes del primer pedido. En la nuestra, no.",
...
C["nl"] = dict(
    ...
    xs=[("Het probleem", "Je inkopers stellen orders samen op een plek die niemand aan jouw kant ziet. In een browsertabblad dat ze elke keer moeten zoeken. Waarin ze elke keer opnieuw moeten inloggen. En het werk zelf gebeurt ergens anders: op een telefoon, in een magazijngang, op een laadperron. Winkelwagens sterven met de sessie. Eén persoon bouwt wat zes tot tien mensen beslissen. En de dubbele bestelling die niemand zag komt terug als retour."),
        ...
        ("Wat een poging kost", "Integratiekosten kwijtgescholden bij jaarplannen. Facturatie start bij livegang, niet bij ondertekening. Pilot van 30 tot 45 dagen met afgesproken meetpunten: haalt hij ze niet, dan betaal je niets."),
    ...
    eu=[("Wederpartij", "Een Ierse vennootschap, opgericht en actief in de Europese Unie naar EU-recht, los van de Amerikaanse entiteit. RocketX Limited, Company No. 344153, statutaire zetel: Derrylahan, Ballyhaunis, Co. Mayo, F35 W667, Ierland."),
    ...
    us=[("Wederpartij", "RocketX LLC, de Amerikaanse vennootschap - een limited liability company naar het recht van Arizona, Business ID 25040687. Hetzelfde platform, dezelfde toezeggingen, recht van je eigen vestigingsplaats."),
    ...
    at=["Verwerkersovereenkomst met de rollen op papier",
        ...
        "Hostingregio’s van je gegevens, schriftelijk vastgelegd",
    ...
    native=[("Nabestellen is een gewoonte, geen zoektocht", "De meeste B2B-orders zijn herhaalorders. In een browser moet de inkoper elke keer de bladwijzer zoeken, de site laden en inloggen. Een app is één tik, al ingelogd — die wrijving verdwijnt bij elke nabestelling, blijvend."),
            ...
            ("En de browser blijft", "Een volwaardige webshop draait in elke browser op elke computer — dezelfde catalogus, dezelfde serverwagen. De apps nemen het over waar de browser niet komt.")],
    ...
    terms=[("Vast tarief, nooit een percentage van je omzet", "De platformkosten stijgen niet omdat je ordervolume is gestegen. Groei wordt niet belast."),
           ...
           ("Integratiekosten kwijtgescholden bij jaarplannen", "De implementatiekosten die zulke projecten normaal vooraf belasten, vervallen."),
           ("Facturatie start bij livegang, niet bij ondertekening", "Je betaalt niets tot het platform live staat en waarde levert."),
           ("Pilot van 30–45 dagen met gezamenlijke ROI-evaluatie", "Een afgebakende beoordelingsperiode met afgesproken maatstaven, vóór een langere verbintenis. Haalt de pilot ze niet, dan stap je eruit en betaal je niets."),
    ...
    nf=[("Je verkoopt uitsluitend aan consumenten", "Het grootste deel van het product is groothandelsmechaniek waar een consumentencatalogus nooit aan komt."),
    ...
    foot="RocketX · Moeiteloos bestellen · www.rocketx.app",
    pg="Pagina",
)
...
C["fr"] = dict(
  ...
  native=[("Le réassort est une habitude, pas une recherche", "La plupart des commandes B2B sont des commandes répétées. Un navigateur oblige l’acheteur à retrouver un favori, charger un site et saisir un numéro de client à chaque fois. Une application, c’est un geste, déjà connecté. La friction disparaît de chaque réassort, pour toute la durée du compte."), ("Une place permanente sur le téléphone de l’acheteur", "Votre icône est sur l’écran d’accueil. Vos concurrents doivent être cherchés et redécouverts."), ("La notification atteint ceux que le courriel n’atteint plus", "Retour en stock, renouvellement de contrat, rappel de réassort : le message arrive directement. Les utilisateurs qui acceptent les notifications restent actifs deux à trois fois plus longtemps."), ("Le travail se fait loin d’un bureau", "Les acheteurs commandent depuis les allées, les quais de chargement et les salons professionnels. L’accès natif à l’appareil photo permet de scanner un code-barres et de réassortir depuis une étiquette de rayon."), ("Elle survit à la perte de réseau", "Les entrepôts aux parois d’acier et les tournées rurales tuent la réception. Catalogue, prix, panier ainsi que vos brochures et vidéos produit restent disponibles en local et se synchronisent au retour de la connexion."),
            ("Et le navigateur reste", "Une boutique en ligne complète tourne dans tout navigateur, sur tout ordinateur — le même catalogue, le même panier côté serveur. Les applications prennent le relais là où le navigateur ne va pas.")],
  ...
  foot="RocketX · Commande fluide · www.rocketx.app",
  pg="Page",
)
...
"es": dict(
  ...
  fg5_meanwhile="Mientras tanto el comercio electrónico B2B de la región crece cerca de un 24 % al año.",
  fg5_n="Gartner para la proporción de autoservicio. Straits Research para el crecimiento del comercio electrónico B2B en América Latina.",
...
/* The tick is DRAWN, not typed. U+2713 is absent from the Latin subsets of
   Space Grotesk, Inter and IBM Plex Mono, so Chrome fell back to whatever
   system face had it - Lucida Grande Bold on this Mac - and embedded that
   one glyph into all five business-case PDFs. A CSS tick has no font
   dependency, so it renders identically on any build machine. */
.attach li::before{content:"";position:absolute;left:1.4px;top:3.3px;width:2.6px;height:5.4px;border:solid var(--blue);border-width:0 1.3px 1.3px 0;transform:rotate(45deg)}
/* exec summary */
...
def cover_strip(lang, idx=None):
    ...
    if idx is None:
        idx = COVER_STRIP_IDX.get(lang, 2)
    st = _g404.STRIPS[idx]
