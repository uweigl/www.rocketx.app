...
C["de"] = dict(
    ...
    xs=[('Das Problem', 'Ihre Einkäufer stellen Bestellungen dort zusammen, wo sie auf Ihrer Seite niemand sieht - in einem Browser-Tab, den sie jedes Mal suchen und in dem sie sich jedes Mal anmelden müssen, während die Arbeit selbst am Telefon stattfindet, im Lagergang, an der Laderampe. Warenkörbe sterben mit der Sitzung, einer baut, was sechs bis zehn entscheiden, und die übersehene Dopplung kommt als Retoure zurück.'),
        ...
        ('Was sich in der Tasche des Einkäufers ändert', 'Ihr Katalog liegt auf dem Homescreen, einen Fingertipp von der Nachbestellung entfernt und bereits angemeldet, statt hinter Lesezeichen und Login. Push holt Einkäufer zurück, wenn Ware wieder da ist oder ein Vertrag ansteht - dort, wo E-Mails gefiltert werden. Barcode-Scan bestellt direkt vom Regaletikett nach, und der Katalog ist lokal gespeichert: Ein Gang ohne Empfang ist immer noch ein Gang, aus dem bestellt werden kann. Ihre Printbroschüren, Produktvideos und weitere Rich-Media-Inhalte liegen in derselben App - das Showroom-Material ist beim Einkäufer, wenn das Gespräch stattfindet. Nativ für iOS und Android, keine Website in einer Hülle. Nachbestellen hört auf, eine Besorgung zu sein, und wird zur Gewohnheit - und genau daher kommt wiederkehrender Umsatz.'),
        ('Warum das haltbar ist', 'Unbegrenzt viele SKUs und Plätze, nie nach Menge berechnet, nie ein Prozentsatz Ihres Umsatzes. Standard-Konnektoren für ERP und CRM inklusive, wobei sich RocketX vor Ihre führenden Systeme setzt, statt sie zu ersetzen - und neben Ihrem bestehenden Webshop läuft, sodass nichts abgeschaltet wird.'),
...
C["es"] = dict(
    ...
    xs=[('El problema', 'Tus compradores arman pedidos donde nadie de tu lado los ve, en una pestaña del navegador que tienen que encontrar y en la que deben iniciar sesión cada vez, mientras el trabajo ocurre en un móvil, en un pasillo de almacén, en un muelle de carga. Los carritos mueren con la sesión, una persona construye lo que deciden entre seis y diez, y el duplicado que nadie vio vuelve como devolución.'),
        ...
        ('Qué cambia en el bolsillo del comprador', 'Tu catálogo está en la pantalla de inicio, a un toque de la recompra y con la sesión ya iniciada, en vez de detrás de un marcador y un inicio de sesión. Las notificaciones traen de vuelta al comprador cuando vuelve el stock o vence un contrato, ahí donde el correo se filtra. El escaneo de códigos recompra directamente desde la etiqueta del estante, y el catálogo queda en caché: un pasillo sin señal sigue siendo un pasillo desde el que se puede pedir. Tus folletos impresos, los vídeos de producto y otro contenido enriquecido viajan en la misma app, así que el material de showroom está con el comprador cuando ocurre la conversación. Nativo para iOS y Android, no un sitio web dentro de una carcasa. Recomprar deja de ser un recado y pasa a ser un hábito, que es de donde sale realmente el ingreso recurrente.'),
        ('Por qué se sostiene', 'SKUs y usuarios ilimitados, nunca medidos, nunca un porcentaje de tus ingresos. Conectores estándar de ERP y CRM incluidos, con RocketX situándose delante de tus sistemas de registro en lugar de reemplazarlos, y funcionando junto a tu tienda web actual, de modo que no se apaga nada.'),
...
C["nl"] = dict(
    ...
    xs=[("Het probleem", "Je inkopers stellen orders samen op een plek die niemand aan jouw kant kan zien, in een browsertabblad dat ze elke keer moeten zoeken en waarin ze elke keer moeten inloggen — terwijl het werk zelf op een telefoon gebeurt, in een magazijngang, op een laadperron. Winkelwagens sterven met de sessie, één persoon bouwt wat zes tot tien mensen beslissen, en de dubbele bestelling die niemand zag komt terug als retour."),
        ...
        ("Wat er verandert in de zak van de inkoper", "Je catalogus staat op het beginscherm, één tik van een nabestelling en al ingelogd, in plaats van achter een bladwijzer en een login. Push haalt inkopers terug wanneer voorraad terug is of een contract afloopt, daar waar mail wordt gefilterd. Barcodes scannen bestelt direct na vanaf een schaplabel, en de catalogus staat lokaal opgeslagen: een gang zonder bereik is nog steeds een gang waaruit besteld kan worden. Je gedrukte brochures, productvideo's en andere rijke content reizen mee in dezelfde app, zodat het showroommateriaal bij de inkoper is wanneer het gesprek plaatsvindt. Native voor iOS en Android, geen website in een omhulsel. Nabestellen houdt op een klusje te zijn en wordt een gewoonte — en daar komt herhaalomzet vandaan."),
        ("Waarom dit standhoudt", "Onbeperkt SKU\'s en onbeperkt plaatsen, nooit per stuk afgerekend, nooit een percentage van je omzet. Standaardkoppelingen voor ERP en CRM inbegrepen, waarbij RocketX vóór je bronsystemen gaat staan in plaats van ze te vervangen — en naast je bestaande webshop draait, zodat er niets wordt uitgezet."),
        ("Wat een poging kost", "Setup kwijtgescholden bij jaarplannen. Facturatie start bij go-live, niet bij ondertekening. Een pilot van 30 tot 45 dagen met afgesproken meetpunten voor elke langere verbintenis."),
    ...
    p1a="B2B-inkoop is online gegaan terwijl de groothandel als geheel nauwelijks groeit. De groei zit niet in meer klanten, maar erin dat dezelfde klanten een groter deel digitaal bestellen — en hun leverancier kiezen op hoe makkelijk dat gaat.",
    ...
    p2a="De meeste groothandels in het middensegment draaien op één van drie dingen: een verouderd ERP-portaal, een algemeen e-commerceplatform dat tot B2B is omgebogen, of orderinvoer per telefoon en mail. Elke variant kost geld dat zelden als post op een factuur staat.",
    costs=[("Catalogusprestaties", "Platformen die voor consumentenhandel zijn gebouwd worden traag bij groothandelscatalogi. Inkopers breken het zoeken af en bellen een vertegenwoordiger — een selfserviceorder wordt daarmee een bemande order."),
           ...
           ("Doorlooptijd van de implementatie", "B2B-trajecten in het middensegment op algemene platformen duren doorgaans vier tot acht maanden of langer tot livegang, met integratiewerk dat gedurende de hele rit wordt gefactureerd."),
    ...
    caps=[("Onbeperkt SKU's, zoeken binnen een seconde", "Of de catalogus nu achthonderd SKU's telt of acht miljoen: filteren en bestellen in milliseconden — en nooit betalen per SKU. Prestaties die andere platformen op deze schaal niet halen."),
          ...
          ("Eerst groothandel, je consumentenkanaal erbij", "Het product is gebouwd rond groothandelsbestellingen. Verkoop je daarnaast rechtstreeks aan consumenten, dan draait dat kanaal op dezelfde catalogus en dezelfde ERP-koppeling in plaats van op een tweede platform met een tweede set productdata."),
    ...
    cma="De toets op gegevens en beveiliging is meestal het traagste deel van een beoordeling, en de gebruikelijke reden is dat een Amerikaanse leverancier moet uitleggen waarom Europese gegevens in Amerika prima zitten. Wij hebben dat vermeden door het niet te doen.",
    ...
    cmn="Geef deze pagina aan wie jullie beveiligingstoets doet. Komt die terug met een vragenlijst, dan beantwoorden we die liever in week één dan in week negen — en dat is wanneer hij meestal binnenkomt.",
    ...
    cart=[("Een half opgebouwde order is onzichtbaar", "Ongeveer 70% van de online winkelwagens wordt verlaten, en op de meeste platformen kan niemand aan jouw kant er één zien voordat het een order wordt. Een vastgelopen winkelwagen ziet er precies zo uit als helemaal geen winkelwagen."),
    ...
    levers=[("Teruggehaalde winkelwagens", "Winkelwagens per maand &times; aandeel dat wordt verlaten &times; gemiddelde orderwaarde &times; het deel dat je buitendienst nu redt omdat hij ze eindelijk ziet."),
            ...
            ("Ordergrootte", "Gemiddelde orderwaarde &times; de stijging die je verwacht doordat de hele catalogus in seconden doorzoekbaar is, plaatsen onbeperkt zijn en er vanaf elk apparaat nabesteld kan worden.")],
    ...
    sb=[("Je huidige webshop blijft draaien", "RocketX gaat ernaast live, niet erbovenop. Er wordt niets uitgezet om ruimte te maken, en geen catalogus hoeft gemigreerd te worden voordat iemand kan bestellen."),
        ...
        ("Geen migratieweekend", "Er is geen migratievenster en geen bestelstop. De faalsituatie die zulke projecten eng maakt — een slechte zondag en maandag rinkelende telefoons — is niet aan de orde, omdat er niets om te zetten valt."),
    ...
    terms=[("Vast tarief, nooit een percentage van je omzet", "De platformkosten stijgen niet omdat je ordervolume is gestegen. Groei wordt niet belast."),
           ...
           ("Facturatie start bij go-live, niet bij ondertekening", "Je betaalt niets tot het platform live staat en waarde levert."),
    ...
    steps=[("1. Doorlichting van de stack", "We brengen de bestaande catalogus, prijsregels, het ERP en de orderstroom in kaart en bepalen wat mee moet en wat blijft."),
           ...
           ("5. Go-live en evaluatie", "Volledige uitrol, met een gezamenlijke ROI-evaluatie tegen de maatstaven die bij de pilot zijn afgesproken.")],
    ...
    tln="De cijfers van concurrenten zijn gangbare benchmarks voor het middensegment tot go-live, uit platform- en bureaugidsen van 2026; de scope verschuift ze beide kanten op en het ERP-werk is de plek waar ze het vaakst uitlopen. Het cijfer van RocketX gaat over een live pilot met een afgebakende groep inkopers, niet over de volledige uitrol. Bij de alternatieven begint de facturatie meestal ruim voordat iemand iets bestelt. Bij ons begint hij bij go-live.",
    ...
    nfn="Zowel aan de groothandel als rechtstreeks verkopen is heel gewoon en hier geen beperking — het consumentenkanaal draait op dezelfde catalogus en dezelfde ERP-koppeling als de groothandel.",
    ...
    quest=["Verandert de platformvergoeding als ons ordervolume groeit?",
           ...
           "Wanneer start de facturatie — bij ondertekening of bij go-live?",
           "Wat gebeurt er met onze gegevens en koppelingen als we weggaan?"],
    ctah="Volgende stap",
    ctap="Een persoonlijke demo op je eigen catalogus, een korte doorlichting van je stack en een pilot met weinig risico — meetbare resultaten binnen weken.",
    contact="app@rocketx.app",
    srch="Bronnen",
